...
export const DeployTargetCard: React.FC = () => {
  ...
  const integrationLevel = target.integration_level ?? 'agent-only';
  const isL1 = !isColima && integrationLevel === 'agent-only';
  ...
  return (
    <div className="rounded-xl bg-surface p-4 shadow-panel">
      ...
      {/* 통합 수준 라디오그룹 (isColima가 아닐 때만 노출) */}
      {!isColima && (
        <div className="p-3 rounded-lg bg-surfaceRaised mb-3">
          <label className="text-label uppercase text-inkFaint mb-1.5 block">
            {t('deployTarget.integrationLevel')}
          </label>
          <div className="flex gap-2" role="radiogroup" aria-label={t('deployTarget.integrationLevel')}>
            {(['agent-only', 'full-stack'] as const).map((level) => {
              const active = integrationLevel === level;
              return (
                <button
                  key={level}
                  type="button"
                  role="radio"
                  aria-checked={active}
                  onClick={() => patchTarget({ integration_level: level })}
                  className={`px-3 py-1.5 rounded-md text-caption transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${
                    active
                      ...
                      : 'bg-surface text-inkMuted hover:brightness-95'
                  }`}
                >
                  {level === 'agent-only' ? t('deployTarget.l1Label') : t('deployTarget.l2Label')}
                </button>
              );
            })}
          </div>
      ...
      {isL1 ? (
        <div className="p-3 rounded-lg bg-surfaceRaised space-y-2">
          <p className="text-caption text-ink">{t('deployTarget.l1Hint')}</p>
          <div className="p-2 rounded bg-surface font-mono text-caption text-primary select-all">
            make kagent-up CONTEXT={target.context}
          </div>
        </div>
      ) : (
        <>
