...
export const LocalInferenceRuntimeCard: React.FC = () => {
  ...
  const updateModel = (model: RuntimeModel, patch: Record<string, unknown>, op: string) =>
    run(`${op}:${model.id}`, async () => {
      const result = await invoke<RuntimeActionResult>('set_omlx_model_settings', {
        request: {
          endpoint,
          model_id: model.id,
          patch,
              ...
              return (
                <div key={model.id} className="rounded-lg bg-surfaceRaised p-3 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-3">
                  ...
                  <div className="flex flex-wrap items-end gap-2">
                    ...
                    <button className={`${buttonClass} bg-surface text-ink`} disabled={!!actionBusy} onClick={() => updateModel(model, { ttl_seconds: ttlValue ? Number(ttlValue) : null }, 'ttl')}>
                      TTL
                    </button>
