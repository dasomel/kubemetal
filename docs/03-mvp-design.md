...
| D40 | (2026-09-23) **학습·서빙 스폰 전 자원 admission 게이트**(D16/D28 확장, GitHub #31/#32; 2026-08-23 stray `git reset --hard`로 유실됐던 8234a3f의 재랜드). D16/D28은 이미 떠 있는 프로세스를 사후에 SIGSTOP하는 반응형 가드레일뿐이라, 이미 memory critical이거나 thermal serious인 상태에서 새 학습/서빙을 **시작하는 것 자체**는 막지 못했다. `guardrails::check_spawn_admission(memory_pressure_level, thermal_state, thermal_pause_enabled)`을 `run_mlx_finetune`/`start_model_serving` 스폰 직전에 배선해 critical 메모리는 무조건 거부(D16과 동일하게 오버라이드 불가), thermal_pause가 켜져 있고 serious/critical이면 거부(D28과 동일 임계선)한다. 판정 불가(thermal None)는 통과시킨다(D22). **"서빙이 학습보다 우선"이라는 정책은 별도 구현이 필요 없었다** — `spawn_guardrail_loop`가 애초에 학습 프로세스에만 붙어 서빙은 자동 SIGSTOP 대상이 아니기 때문이다(구조 자체가 그 정책이었다는 사실을 이번에 처음 명시했다). 스폰 배선(`run_mlx_finetune`/`start_model_serving` 호출부)은 이 재랜드 스코프 밖으로 남겨뒀다 — 원본 이슈의 별도 커밋(#32)과 동일하게, 순수 함수만 우선 들여온다. |
