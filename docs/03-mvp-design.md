...
| D37 | (2026-09-08) **MLX 파인튜닝 산출물 무결성 매니페스트.** MLX 파인튜닝이 성공 종료한 뒤 어댑터 출력 디렉터리에 `manifest.json`을 기록한다. 매니페스트는 스키마 버전·UTC 생성 시각·런타임·학습 실행이 알고 있던 베이스 모델과 일반 파일의 경로/바이트/SHA-256을 담으며, `manifest.json` 자신·심볼릭 링크·하위 디렉터리는 제외한다. 기록 실패는 가중치 결과를 실패로 바꾸지 않고 학습 경고 로그로 남긴다. |
| D38 | (2026-09-10) **Kubernetes 보안 베이스라인(OpenForge 프로필 `standard`), k3s 노드 범위.** MLflow/SeaweedFS/Prefect(+ init 컨테이너)에 비루트 고정 UID/GID/fsGroup 65532, `seccompProfile: RuntimeDefault`, `allowPrivilegeEscalation: false`, `capabilities: drop: [ALL]`, `readOnlyRootFilesystem: true`(각 컨테이너가 실제로 필요로 하는 쓰기 경로만 emptyDir로 예외)를 적용한다. 앱별 기본 차단 NetworkPolicy(`scripts/k8s/network-policies.yaml`) — 네임스페이스 전체가 아니라 `app` 라벨 범위인 이유는 `default` 네임스페이스가 이 kustomization이 의도적으로 제외한 다른 워크로드와 공존하기 때문(D26). k3s 임베디드 kube-router netpol이 실제로 강제됨을 실측했고(allow/deny 쌍), `kubectl port-forward`와 kubelet readinessProbe 둘 다 이 강제 경로를 우회함도 실측했다 — 그래서 "내부/외부 노출 분리"는 이 토폴로지에서 NetworkPolicy로 의미 있게 그을 경계가 아니라고 정직하게 기록했고(파드 간 lateral movement 통제가 실제 의미 있는 것), PyPI egress는 k3s netpol의 L3/L4 한계상 IP 대역(파드/서비스 CIDR 제외)까지만 좁혔다. 네이티브 LSM(AppArmor)은 애너테이션 없이 이미 강제되고 있음을 실측(`docker-default` 프로파일)하고 손대지 않았다. 기존 PVC(prefect-data)에 fsGroup을 소급 적용하려다 kubelet이 이미 마운트된 볼륨의 소유권을 재조정하지 않는 것과 부딪혀 Deployment를 `Recreate` 전략으로 바꾸고 노드에서 1회성 `chown`을 수행했다. 근거와 판정표 전체는 ADR-0003(`docs/adr/0003-k8s-security-baseline.md`). |
| D39 | (2026-09-10) **데이터 수집 재현성 및 불변 데이터셋 매니페스트.** 데이터 수집 파이프라인(`ingest_host.py`) 완료 시 수집 산출물 디렉터리에 `dataset-manifest.json`을 기록한다. 매니페스트는 스키마 버전·불변 데이터셋 버전(`dataset_version`, 정규화된 입력 소스 URI·문서별 콘텐츠 해시·청크 크기·오버랩·임베딩 모델 ID·인덱스 스키마 버전의 SHA-256)·추출 시각(UTC)·청킹 파라미터·임베딩 모델 및 런타임/디바이스·LanceDB 컬렉션 메타데이터(컬렉션명·인덱스 스키마 버전·벡터 차원수)를 포함한다. 런타임에 실측할 수 없는 값은 지어내지 않고 알 수 없음으로 표기한다(D22). 기존 컬렉션 재수집 시 동일 데이터셋 버전은 멱등하게 수렴하며, 서로 다른 버전의 기존 컬렉션 덮어쓰기는 기본적으로 차단하고 명시적인 `--overwrite-existing` 옵트인 플래그를 요구한다. LanceDB 인덱스 레코드에도 `dataset_version`을 기록해 인덱스와 매니페스트 간 정합성을 유지한다. |
| D40 | (2026-09-23, 2026-09-23 재검토로 배선 여부 문구 정정) **학습·서빙 스폰 전 자원 admission 게이트 — 판정 로직만 존재, 아직 미배선.** D16/D28은 이미 떠 있는 프로세스를 사후에 SIGSTOP하는 반응형 가드레일뿐이라, 이미 memory critical이거나 thermal serious인 상태에서 새 학습/서빙을 **시작하는 것 자체**는 막지 못한다는 문제(D16/D28 확장, GitHub #31/#32; 2026-08-23 stray `git reset --hard`로 유실됐던 8234a3f의 재랜드)에 대해, 순수 함수 `guardrails::check_spawn_admission(memory_pressure_level, thermal_state, thermal_pause_enabled)`를 도입했다 — critical 메모리와 측정 실패(unknown) 메모리는 무조건 거부(D16과 동일하게 오버라이드 불가, unknown은 이 함수가 D22 원칙의 예외로 fail-closed 처리한다 — 근거는 함수 자체의 doc comment), thermal_pause가 켜져 있고 serious/critical이면 거부(D28과 동일 임계선), thermal 판정 불가(None)는 통과시킨다(D22). **이 함수는 현재 어떤 스폰 경로에서도 호출되지 않는다** — `run_mlx_finetune`/`start_model_serving`에 실제로 배선하는 일(원본 이슈의 별도 커밋 #32/53464bb)은 이번 재랜드 스코프 밖으로 명시적으로 남겨뒀다. 재랜드 직후 독립 리뷰(2026-09-23)에서 이 표와 코드 주석이 "스폰 직전에 부른다"처럼 이미 배선된 것으로 잘못 서술돼 있던 것을 지적받아 정정했다 — **본 함수의 존재는 게이트가 작동 중이라는 뜻이 아니다.** **"서빙이 학습보다 우선"이라는 정책은 배선이 들어갈 때도 별도 구현이 필요 없다** — `spawn_guardrail_loop`가 애초에 학습 프로세스에만 붙어 서빙은 자동 SIGSTOP 대상이 아니기 때문이다(구조 자체가 그 정책이었다는 사실을 이번에 처음 명시했다). |
...
| D25 | (2026-07-25) **폐쇄망 기동 검증은 호스트 네트워크를 끊지 않는다** — 개발 세션 자체가 네트워크에 의존하므로 실용적이지 않다. 대신 `make verify-airgap`(`scripts/airgap/verify_offline_images.sh`)이 각 이미지로 `imagePullPolicy: Never` 프로브 파드를 띄운다. 이 정책은 kubelet에 "레지스트리를 절대 조회하지 말라"를 강제하므로, 이미지가 런타임 저장소에 없으면 `ErrImageNeverPull`로 즉시 실패하고 있으면 기동을 시도한다 — 레지스트리 접근 0 조건을 네트워크를 건드리지 않고 재현하는 것이다. 컨테이너의 이후 종료 코드는 판정 대상이 아니다(관심사는 "이미지를 가져올 필요가 있었는가"뿐). 검증 대상은 수집과 **같은 두 출처**에서 파생한다: 매니페스트의 `image:` + `scripts/airgap/images-helm.txt`(매니페스트가 선언하지 않는 이미지의 단일 출처, 수집·검증 스크립트가 공유해서 읽는다). 전용 네임스페이스에서 돌리고 `trap`으로 정리하므로 기존 워크로드에 영향이 없다. 실기기 결과(2026-07-25): 12개(매니페스트 5 + 비매니페스트 7) 전부 PASS. **음성 대조 필수** — 로컬에 없는 태그로 같은 프로브를 돌려 `ErrImageNeverPull`이 검출되는지 확인해야 한다(확인함). 무조건 통과하는 검증은 증거가 아니다. **digest lock (2026-09-08 개정)**: `docker save/load`는 registry `RepoDigests`를 보존하지 않는다. 수집은 `digests.lock`에 `<image:tag> <RepoDigest|unverified> <image ID>`를 기록한다. 둘째 필드는 pull 시점 provenance이고 locally-built 이미지는 `unverified`를 정직하게 남긴다. 셋째 필드는 `docker image inspect .Id`의 content-addressed config digest라 save/load 뒤에도 남으며, 설치는 load 전 daemon cache와 load 직후 tag의 ID를 이 값과 대조한다. lock 누락·빈 줄 외 형식 오류·ID 불일치는 Helm/apply 전에 중단하되, `unverified` provenance 자체는 일치한 ID를 막지 않는다. `AIRGAP_ALLOW_UNLOCKED=1`만이 이 검증을 명시 경고와 함께 건너뛰는 예외다. 캐시 재사용은 이전 세 필드 lock 항목을 그대로 계승한다. **한계**: kubelet의 이미지 조회는 완전히 배제하지만, 파드가 기동한 뒤 외부로 나가는 트래픽(라이선스 확인·텔레메트리 등)은 이 프로브로 잡히지 않는다. |
