...
| ID | 결정 |
|----|------|
| D1 | (2026-07-25 개정) 포트: MLflow 5001(호스트 포워딩, AirPlay가 5000 점유), SeaweedFS Filer UI 8888, S3 API 8333, 서빙 8080(`suggest_serving_port`가 8080~8099를 순회), Prefect 서버 4200(D19), **kagent UI 8090**. kagent UI를 8080으로 포워딩하면 모델 서빙과 정면 충돌한다 — `make forward`는 `8090:8080`으로 매핑하고 UI 링크(`KagentOpsView`, 파이프라인 스테이지)도 `http://127.0.0.1:8090`을 쓴다(서빙 URL과 마찬가지로 `localhost` 금지, mistakes-log 2026-07-21).  **(2026-08-06 개정 — 이 표는 "우리가 쓰기로 한 포트"이지 "비어 있는 포트"가 아니다.)** 실측: 무관한 로컬 Docker 컨테이너 둘이 이 표의 포트를 점유하고 있었다 — `narwhal-airgap-registry`가 5001(MLflow), `egov-compat-checker`가 8080(서빙). 둘 다 IPv6 와일드카드(`*:포트`)에 바인딩하는데, 이는 더 구체적인 리스너가 없을 때 IPv4 루프백 연결까지 받아간다. 즉 **포트가 열려 있다고 해서 우리 서비스가 아니다** — `kubectl port-forward`가 127.0.0.1에 더 구체적으로 바인딩하면 우리 것이 이기지만, 포워드가 죽는 순간 남의 컨테이너가 같은 주소를 조용히 이어받는다. 그래서 헬스 판정은 "응답이 오는가"가 아니라 "기대한 서비스가 200으로 답하는가"여야 한다(`check_health`가 200을 요구하도록 수정, `check_serving_health`가 이미 쓰던 기준과 통일). 같은 이유로 모든 로컬 URL은 `127.0.0.1`이며 `localhost`를 쓰지 않는다 — macOS에서 `localhost`는 `::1`로 먼저 풀리고, 그 주소 대역이 바로 저 컨테이너들이 잡고 있는 곳이다. **(2026-08-06 개정 2 — 표의 숫자는 우선 시도값이지 배정 결과가 아니다.)** 위 사실의 귀결로, 고정 포트를 전제하면 남이 그 포트를 쥔 환경에서 앱이 통째로 어긋난다. 이제 `services/ports.rs`가 (키 → 우선 포트, 대체 범위) 표와 **실제 배정 결과**를 함께 소유하고, 포워딩·서빙 모두 기동 직전에 비어 있는지 검사해 막혀 있으면 범위 안의 다음 포트를 잡는다. 탐지는 `0.0.0.0`과 `127.0.0.1` **양쪽** 바인딩을 요구한다 — 루프백만 보면 와일드카드로 잡은 프로세스를 "비어 있음"으로 오판한다(`is_port_free_detects_wildcard_holder` 테스트가 이 회귀를 고정한다). 소비자는 상수를 버리고 `ports::local_url(key)`에서 파생하며(`access.rs`·`prefect.rs`·`modelhub.rs`·`rag.rs`·`data_ingest.rs`), 프런트는 `get_service_access`의 `url`과 새 `get_host_ports`로 링크를 만든다 — 컴포넌트에 숫자를 박아두면 대체 포트가 잡혔을 때 링크만 조용히 옛 곳을 가리킨다. 자식 파이썬 프로세스는 Rust가 이미 `--remote-url`/`PREFECT_API_URL`을 명시로 넘기므로 자동으로 따라온다. 포워딩 배정은 반드시 **기존 포워드 정리(reap) 이후**에 하고, 우선 포트가 풀릴 때까지 최대 2초 기다린다 — 우리가 방금 죽인 포워드를 남의 점유로 오인하면 재시작마다 포트가 한 칸씩 밀린다(포트 크리프). 대체 포트가 선택되면 반환 문자열에 명시한다(조용히 바꾸지 않는다 — 사용자의 `.dvc/config`나 북마크가 D1 포트를 가정하고 있을 수 있다).|
...
| D33 | (2026-08-05) **kagent Helm 차트 버전의 단일 출처는 `scripts/helm/kagent-version.txt`다.** kagent 설치가 Makefile(`kagent-up`)과 앱(`install_kagent`) 두 경로가 되면서 버전 리터럴이 두 곳에 생길 위기였다 — Makefile은 `$(shell cat …)`, Rust는 `include_str!`로 같은 파일에서 파생한다(파생 가능하므로 "어긋나면 실패하는 테스트"보다 파생을 택함; `kagent_version_is_trimmed_and_non_empty` 테스트가 파일 형식만 고정). 에어갭 자산 목록(`STATIC_AIRGAP_TARGETS`, images-helm.txt)의 kagent 버전은 별개 도메인(오프라인 번들 추적)이라 이 결정의 범위 밖 — 두 도메인 간 드리프트는 `make verify-airgap`이 실측으로 잡는다. 함께 도입: `install_kagent`(앱 내 설치 버튼, D30 L1 호환이라 게이트 없음 — kagent CRD 설치는 클러스터→Mac 의존을 만들지 않는다), `KagentDiagnosticReport.kagent_installed`, kagent 코드의 `commands/kagent.rs` 분리(colima.rs 875→595줄), 진단 배지(탭 미방문 시 미표시·조회 시각 명시 — D22 계열, 폴링 신설 없이 기존 조회의 push 브로드캐스트만). 검증(2026-08-05): 패키징 앱에서 미설치 감지→설치 클릭→helm 릴리스 생성·8파드 기동 실측, 번들 내 scripts/helm 리소스 존재 확인. **(2026-08-05 개정 — 설치 대상은 저장된 DeployTarget이 아니라 화면의 kubeconfig 선택기다.)** 최초 구현은 `install_kagent`가 저장된 DeployTarget(D26)을 대상으로 삼았는데, 설치 버튼이 사는 진단 패널은 통째로 드롭다운(`selectedContext`) 축이고 형제 동작인 `toggle_kagent_agent`도 드롭다운을 쓴다 — 한 화면에서 진단·에이전트는 narwhal로 가고 설치만 colima로 가, 사용자는 helm이 성공 출력을 냈는데도 재조회에서 영원히 "미설치"를 봤다(실측 2026-08-05: `helm ... [colima] returned: STATUS deployed REVISION 5`, 화면은 `context [narwhal] ... 0 pod(s)`). 개정: `install_kagent`가 `context: Option<String>`을 받아 드롭다운 값을 쓰고, 생략 시에만 DeployTarget으로 폴백한다(하위 호환). 교훈은 "설치 대상"이라는 사실이 프런트 선택기와 백엔드 설정 두 곳에 있었다는 것 — 같은 사실 두 곳 금지의 UI판이다. **(2026-08-05 개정 2 — CRD 차트가 선행 단계다.)** kagent는 `kagent-crds` + 본 차트 2개로 설치된다(`docs/08` §9·§38이 처음부터 그렇게 적어뒀다). 그런데 `Makefile kagent-up`과 `install_kagent` 둘 다 본 차트만 설치했고, colima는 2026-07-23 수동 설치분 CRD 9종이 남아 있어 이 누락이 1년 가까이 가려져 있었다. CRD 없는 클러스터(narwhal 실측)에서는 helm이 렌더 단계에서 `no matches for kind "Agent" in version "kagent.dev/v1alpha2" ensure CRDs are installed first`로 죽는다 — 즉 **최초 설치만 실패하고 재설치는 성공하는** 형태라 기존 클러스터에서는 절대 재현되지 않는다. 개정: **세 경로 모두** `kagent-crds`를 먼저 `helm upgrade --install`한다 — `Makefile kagent-up`(`KAGENT_VERSION` 변수로 두 호출이 같은 파일을 읽는다), `install_kagent`(`helm_upgrade_kagent(context, chart, values, install)` 한 헬퍼가 두 차트를 다 조립), 그리고 폐쇄망 경로(`download_airgap_bundle.sh`가 CRD 차트를 받고 `install_from_airgap.sh`가 먼저 설치, `STATIC_AIRGAP_TARGETS`에 자산 행 추가 — 번들에 빠져 있으면 상태 화면이 "완비"라고 거짓말한다). 함께 잡은 드리프트: `images-helm.txt`가 `postgres:16-alpine`을 수집했으나 차트 0.9.12의 values는 `database.postgres.bundled.image.tag: "18.3-alpine"`이라(차트 values + colima/narwhal 러닝 파드 3중 실측) 폐쇄망 번들이 **아무도 쓰지 않는 postgres를 받아오고 정작 필요한 것은 빠져 있었다** — `static_airgap_targets_match_images_helm_txt`는 두 목록의 일치만 보므로 둘이 함께 틀린 이 경우를 잡지 못한다(태그의 출처는 차트다). 실측(2026-08-05, narwhal): 개정 전 렌더 실패 → 개정 후 `kagent-crds`(rev 1) + `kagent`(rev 1) 배포 성공, CRD 9종 생성 확인. |
| D34 | (2026-08-06) **로컬 kagent가 외부 클러스터를 진단한다 — 외부에는 읽기 전용 ServiceAccount 하나만 둔다(D30 L0).** 발단은 "narwhal을 kagent로 진단"이었고, 그 클러스터에 kagent을 설치하는 길이 두 겹으로 막혀 있었다: 노드가 `cr.kagent.dev`에 `network is unreachable`이고 레지스트리를 `ghcr.io`(공개·multi-arch·arm64 확인)로 바꿔도 controller/tools/kmcp가 전부 ImagePullBackOff이며, 워커 3대의 여유 CPU가 총 480m(89~94% 소진)이라 차트 기본 요청량 ~700m이 들어가지 않는다. **"에이전트만 외부, 본체는 로컬"은 kagent 구조상 불가능하다** — `Agent`는 CR일 뿐이고 이를 파드로 만드는 건 같은 클러스터의 컨트롤러다(로컬 컨트롤러는 로컬 API 서버만 감시한다). 대신 **도구 서버를 로컬에 하나 더 띄우고 원격 자격증명을 준다**: `RemoteMCPServer`는 URL을 가리키는 범용 CR이고(`protocol: STREAMABLE_HTTP`, 도구는 status에 자동 discovery), `kagent-tools`는 `/tool-server --port 8084` 단일 바이너리이며 호출 시점에 kubectl을 실행하므로 `KUBECONFIG` 하나로 대상 클러스터가 바뀐다. 구성: 원격에 `kagent-remote-reader` SA + 내장 `view`(secrets 미포함) + 클러스터 범위 읽기 ClusterRole(nodes/namespaces/PV/StorageClass/CRD), 만료 없는 `kubernetes.io/service-account-token` Secret. 로컬 `default` ns에 그 kubeconfig를 마운트한 도구 서버 Deployment/Service, `kagent` ns에 `RemoteMCPServer` + `Agent` CR 2개. **워크로드가 `kagent` ns가 아니라 `default`에 있는 이유**는 helm 소유권이다 — mistakes-log 2026-08-05의 `kagent-openai` 사고와 같은 계열을 피한다. 대상은 상수가 아니라 `REMOTE_CONTEXT` 인자다(D26 계승). 호스트 포트를 쓰지 않으므로 D1 변경 없음. 실측(2026-08-06, narwhal): 로컬 파드에서 `k8s_get_resources(nodes)`가 narwhal 노드 6대(192.168.56.10~23, Ubuntu 26.04)를 반환, RemoteMCPServer가 도구 124종 discovery + `Accepted=True`, `auth can-i`로 읽기 허용·secrets/create/delete 거부 확인. 되돌리기는 `make remote-reader-down`이 양쪽에서 제거한다.  **(2026-08-06 후속 실측 — L1이 왜 이 클러스터에서 안 되는지 끝까지 확인했다.)** 이미지 실패의 원인은 노드의 인터넷 부재가 아니라 **레지스트리 미러**였다: 같은 Mac이 띄운 Docker 컨테이너 `narwhal-airgap-registry`가 출처 호스트별 경로(`ghcr.io/…`, `quay.io/…`)로 101개 저장소를 미러링하고 있고, 미러에 없는 이미지는 `not found`, 미러 항목 자체가 없는 호스트(`cr.kagent.dev`)는 `network is unreachable`로 나온다 — 두 메시지 모두 원인을 다른 곳으로 가리킨다. 필요한 이미지 6종(kagent controller/app/ui/tools + kmcp + postgres:18.3-alpine)을 미러에 push하자 **ImagePullBackOff 없이 kagent 코어 5파드가 전부 Ready**가 됐다(실측). 그럼에도 L1은 성립하지 않았다: 코어가 남아 있던 480m을 그대로 소진해 워커가 99~100%가 되고, 에이전트(각 50m)는 한 개도 스케줄되지 않으며, 에이전트 기동에 필요한 `kagent-openai` 시크릿은 모델 백엔드 지정을 요구하는데 외부 클러스터가 Mac의 MLX를 보게 하는 것은 L1 정의 위반(L2 영역)이다. 결론: **이 클러스터에서 L1은 이미지가 아니라 용량 때문에 불가능하다.** 철수 후 워커는 92~94%로 복귀했고 L0는 그대로 동작한다. 미러에 올린 이미지는 남겨두므로 여유가 생기면 재설치는 즉시 가능하다. push는 Docker Desktop 데몬이 Mac의 `127.0.0.1:5001`에 닿지 못해 무한 대기하므로 레지스트리와 같은 브리지 네트워크의 `crane` 컨테이너로 해야 한다. **(2026-08-07 확장 — 터널 방식 API 주소 지원.)** 카카오 클라우드처럼 bastion/SSH 터널을 거치는 클러스터는 kubeconfig의 server가 `https://127.0.0.1:6443`이다. 그 주소는 Mac 호스트에서만 의미가 있어 도구 서버 파드에서는 자기 루프백이 되고 연결이 실패한다(실측: HTTP=000). 파드는 D10 브리지로 Mac 호스트에 닿으므로(`host.lima.internal:6443` 실측 HTTP=200) `setup-remote-reader.sh`가 **루프백 API를 감지해 파드용 주소를 브리지 이름으로 바꾼다**. 인증서 SAN에는 그 이름이 없으므로 `tls-server-name`으로 SAN에 실제로 들어 있는 이름(`kubernetes` 등)을 지정해 **검증을 유지한다** — 자격증명 경로에 `insecure-skip-tls-verify`는 쓰지 않는다(D21). SAN에서 쓸 이름을 못 찾으면 검증을 포기하는 대신 그 자리에서 멈춘다. 실측(2026-08-07, 카카오): 도구 호출이 카카오 노드 6대(172.17.0.x, Ubuntu 24.04)를 반환하고, colima의 `remote-cluster-agent`가 로컬 MLX로 한글 진단을 회신했다. **제약: 원격 대상은 한 번에 하나다** — Secret/Deployment/CR이 단수라 다른 컨텍스트로 다시 실행하면 이전 연결을 대체한다.|
| D35 | (2026-09-05) **oMLX가 선호 로컬 추론 런타임이고, `mlx_lm.server`는 계속 폴백이다.** `services/runtime_adapter.rs`의 어댑터 계층(`OmlxAdapter`/`MlxLmAdapter`)이 발견·능력(멀티모델/파닝/TTL/티어드 KV 캐시 등)을 선언하고, `get_local_inference_status`는 oMLX가 설치돼 있으면 그것을 우선하고 아니면 mlx-lm으로 떨어진다 — mlx-lm을 강제 의존으로 만들지 않는다(docs/11). **관리형 oMLX는 항상 loopback 바인드 + KubeMetal 소유 PID 추적이다**: `build_omlx_command`는 `--host 127.0.0.1`을 항상 명시하고(`services/local_inference.rs`), `start_local_inference_runtime`은 이미 리스닝 중인 포트를 발견하면 소유권을 가져가지 않으며(`probe_live_runtime`으로 선점 확인), `LocalInferenceState.managed`가 KubeMetal이 스폰한 PID만 추적해 `stop_local_inference_runtime`이 그 PID에만 SIGTERM을 보낸다(D22 계열 — 남의 프로세스를 건드리지 않는다). **K3s→host 릴레이(`local_inference_bridge.rs`)는 KubeMetal 소유 TCP 릴레이이고, 타깃은 항상 `127.0.0.1:<runtime-port>`다**(`target_addr`가 포트와 무관하게 `Ipv4Addr::LOCALHOST`로 고정). 릴레이의 바인드 주소는 임의의 private/link-local이 아니라 **D10 배포 대상이 실측 검증한 브리지 주소와 정확히 일치해야 한다** — `allowed_private_bind_host`가 loopback이거나 `services::deploy_target::active_verified_bridge_host()`(`BridgeState::Verified{host}`)와 일치하는 주소만 허용한다(2026-09-05 보안 리뷰로 강화 — 이전에는 사설/link-local 대역이면 전부 허용해, 인증 없는 릴레이가 colima vz 브리지가 아니라 사용자의 실제 LAN 인터페이스에 뜰 수 있었다). colima의 기본 브리지(`BridgeState::KeepBase`)는 DNS 이름(`host.lima.internal`) 기반이라 이 코드베이스에 검증된 숫자 주소가 저장된 적이 없어, 현재는 릴레이의 non-loopback 바인드가 colima 기본 경로에서는 성립하지 않는다. 다만 colima에서는 릴레이 자체가 불필요하다 — vz 게이트웨이(`host.lima.internal` → 192.168.5.2)는 호스트 loopback에 바인드된 서비스에 닿는다는 것이 D32에서 실측됐으므로(127.0.0.1:8080 서빙을 kagent 파드가 호출), oMLX도 같은 경로로 도달한다. 릴레이는 외부 클러스터 L2(D30)의 IP 브리지 전용이다. **(2026-09-06 추가, 실측)** `RuntimeLaunchConfig.memory_guard_tier`는 `Option<String>`으로 `{off,safe,balanced,aggressive}`만 허용한다 — `omlx serve --memory-guard`는 값 없이 부르면 argparse에서 죽고(설치된 0.6.4에서 실측), 파닝은 launch 플래그가 아니라 기동 후 `set_omlx_model_settings_sparse`의 admin API 호출이다(`--pin` 플래그 자체가 없음). `/admin/api/*`는 `/v1/*`의 bearer 토큰이 아니라 `omlx_admin_session()`이 `/admin/api/login`으로 교환하는 세션 쿠키로 인증한다(같은 유효 키를 bearer로 `/admin/api/models`에 보내면 401 실측). **(2026-09-06 실기기 검증 완료, §5 항목 6)** M4 Pro/64GB/macOS 26/oMLX 0.6.4/colima(vz)에서 docs/11 런북이 통과했다 — MLX Studio에서 관리형 기동(PID 53467, `--host 127.0.0.1`), 모델 6종 발견·Load/Pin(0.5B, 실측 457.8MB), K3s→host 프라이빗 브리지(127.0.0.1:18000→127.0.0.1:8000, 콜리마 VM 내부에서 `host.lima.internal:18000`/`:8000` 양쪽 health 200), 동시성 1/2/4/8 스트리밍 벤치마크(32건 무오류), 2모델 전환 하니스(성공, 7B 콜드 로드 2675ms), SIGTERM 단일 PID 종료까지 확인했다(증거: `evidence/local-inference/20260906-ondevice/`, docs/12 §12). **미검증으로 남은 것**: 실제 외부망 차단 재실행, warm/ssd-restore 캐시 라벨, 외부 클러스터 L2 브리지, 이 UI를 통한 mlx-lm 폴백 경로. |
...
| D36 | (2026-08-25) **학습·서빙 스폰 전 자원 admission 게이트**(D16/D28 확장, GitHub #31/#32). D16/D28은 이미 떠 있는 프로세스를 사후에 SIGSTOP하는 반응형 가드레일뿐이라, 이미 memory critical이거나 thermal serious인 상태에서 새 학습/서빙을 **시작하는 것 자체**는 막지 못했다. `guardrails::check_spawn_admission(memory_pressure_level, thermal_state, thermal_pause_enabled)`을 `run_mlx_finetune`/`start_model_serving` 스폰 직전에 배선해 critical 메모리는 무조건 거부(D16과 동일하게 오버라이드 불가), thermal_pause가 켜져 있고 serious/critical이면 거부(D28과 동일 임계선)한다. 판정 불가(thermal None)는 통과시킨다(D22). **"서빙이 학습보다 우선"이라는 정책은 별도 구현이 필요 없었다** — `spawn_guardrail_loop`가 애초에 학습 프로세스에만 붙어 서빙은 자동 SIGSTOP 대상이 아니기 때문이다(구조 자체가 그 정책이었다는 사실을 이번에 처음 명시했다). |
| D37 | (2026-08-25) **GPU 텔레메트리 백엔드 공통 인터페이스**(GitHub #1). `metrics.rs`에 `GpuTelemetryBackend` trait(`name()`, `read() -> (util%, mem_gb)`)을 신설하고 `AppleMetalBackend`로 구현, `SystemMetrics.gpu_backend`가 그 `name()`을 그대로 싣는다. NVIDIA(원격 클러스터, Narwhal이 source-of-truth)는 **실기기가 없어 구현하지 않았다** — trait만 남기고 `NvidiaBackend`는 만들지 않는다(D22, 없는 백엔드를 지어내지 않는다). 장비를 확보하면 이 trait을 구현하는 새 백엔드를 추가하면 된다. |
| D38 | (2026-08-25) **학습 산출물 무결성 매니페스트 + 삭제 가드**(GitHub #22/#33). 학습이 `"done"`으로 끝난 경우에만(실패 시 미생성) adapter 디렉터리 파일들의 sha256을 `manifest.json`으로 기록한다(`write_training_manifest`, 서브디렉터리·심볼릭 링크 제외, 파일명 정렬로 재현성 확보, 매니페스트 자신은 제외). `manifest_verification_status(adapter_dir)`가 그 매니페스트를 재검증해 `missing`(매니페스트 없음, 구버전 산출물 또는 실패한 학습)/`verified`(해시 일치)/`corrupt`(불일치)를 판정한다. `is_adapter_safe_to_delete(adapter_dir, mlx_state)`는 현재 서빙 중이거나 `last_known_good_serving`(D36 인접, `#12`)로 기록된 adapter를 삭제 금지 대상으로 판정한다 — **실제 삭제 실행 기능은 이 스코프에 없다**, 판정 함수만 제공한다(파일 삭제는 되돌릴 수 없어 별도 결정 사항으로 남긴다). |
| D39 | (2026-08-25) **파괴적 작업 대상 확인 + colima 재진입 방지**(GitHub #18). `describe_deploy_operation` IPC 커맨드가 `start_cluster`/`stop_cluster`/`provision_mlops_stack`/`install_kagent` 각각에 대해 실행 전 대상(컨텍스트/네임스페이스/액션)을 조회할 수 있게 한다 — 알 수 없는 액션은 에러를 반환하고 요약을 지어내지 않는다(D22). `colima start`는 `ColimaState.lifecycle_in_progress`로 재진입을 거부한다 — "colima is not reentrant"(본 문서 "What bites here")가 이제 주석이 아니라 코드로 강제된다. 멀티 컨텍스트 동시 isolation·stale context 검증은 다중 세션/프로필 모델 자체가 없어 이 스코프 밖으로 남겨뒀다. |
| D40 | (2026-09-08) **CI/릴리스 워크플로 공급망 하드닝**(GitHub #36). CI/릴리스 워크플로의 모든 `uses:` 액션을 태그가 아닌 full-commit-SHA로 고정하고(`git ls-remote --tags`로 후보를 찾고 `gh api repos/<owner>/<repo>/commits/<sha>`로 각각 재검증), 각 job 첫 스텝에 `step-security/harden-runner`를 추가했다. **audit 모드는 과도기다**: 이 리포는 이제 origin에 push되어 실제 CI가 초록으로 돈다(main 최신 커밋 기준 `gh run list` 확인), 하지만 harden-runner의 egress 로그를 아직 검토한 적이 없어 block으로 바로 승격하지 않고 audit(로깅만)으로 시작한다 — 몇 차례 실제 실행 로그를 검토해 baseline을 확정한 뒤 block으로 승격하는 것이 후속 작업이다(**이 결정은 #36을 완전히 닫지 못한다**). **annotated tag 고정의 함정**: `git ls-remote --tags`는 annotated 태그에 대해 태그 객체 SHA와 `<tag>^{}`로 표시되는 역참조 커밋 SHA 두 줄을 함께 돌려주는데(예: `pnpm/action-setup`, `Swatinem/rust-cache`), `^{}` 줄을 놓치면 `uses:`에 커밋이 아니라 태그 객체를 고정하게 되어 `gh api .../commits/<sha>`가 그 SHA를 커밋으로 인식하지 못한다 — 이번 작업은 매 SHA를 이 API로 사전 검증해 이 실수를 피했다(이전에 유실된 커밋에서 실제로 발생했던 실수, 본 문서 개정 이력 참고). **네거티브 픽스처**(`.github/workflows/supply-chain-fixture.yml`, `fixtures/build-script-egress-probe/`)는 기존 구현을 그대로 유지했으나 이번 작업에서도 `workflow_dispatch`로 실제 실행해 GitHub-hosted 러너에서 green을 확인하지는 않았다 — 실행하지 않은 것을 성공으로 보고하지 않는다(D22). **신규 crate 채택 리뷰 정책**(direct/transitive 구분 없이 Cargo.lock diff에 새로 등장하는 모든 crate 대상): (1) build.rs/proc-macro 보유 여부 확인, (2) 게시 후 최소 7일 경과 또는 그 미만이면 리뷰 근거를 커밋 메시지/PR에 명시, (3) crates.io 소유자/다운로드 이력에 이상 징후(신규 소유자 이전, 급격한 버전 점프) 없는지 확인 — 을 요구한다. 신규 git/미승인 registry 소스는 `deny.toml`의 `[sources]`가 CI에서 자동 차단한다. **롤백/격리 절차**: yanked crate(즉시 악성은 아님, triage 대상)는 `cargo tree -i <crate>@<version>`으로 direct/transitive 경로 확인 → 가능하면 `cargo update -p <crate> --precise <ver>`, 안 되면(manifest 제약 문제) `Cargo.toml`의 버전 제약 또는 상위 크레이트 자체를 교체 → Cargo.lock 재생성 → `make verify` + `make supply-chain-check` + `make vuln-check` 전체 재통과 → SBOM 재생성. 침해 의심(공식 RUSTSEC 공지 또는 신뢰할 수 있는 보고)은 yanked보다 무겁게 다뤄 해당 크레이트를 즉시 quarantine하고 그 크레이트가 포함된 기존 릴리스 산출물의 배포 중단을 검토한다(release 격리는 이 스코프에서 절차만 문서화, 자동화는 안 함). release.yml은 build(contents: read + `persist-credentials: false`)/publish(contents: write) job으로 권한을 분리했다 — build가 만든 산출물(zip, `SHA256SUMS.txt`, `release-manifest.json`)은 `actions/upload-artifact`로 넘기고 publish는 `actions/download-artifact`로 받아 릴리스를 생성만 한다. publish job은 checkout을 하지 않으므로 `gh release create`가 대상 저장소를 추론하지 못해 `GH_REPO` env로 명시했다. **cargo-deny 도입 + report-only 배선(실측 2026-09-08)**: trivy가 보지 못하는 yanked crate·미승인 소스를 잡기 위해 trivy와 별개로 도입했다(`deny.toml`, `make supply-chain-check`). 현재 `check advisories sources`를 돌리면 취약점(quick-xml/plist 경로 2건 — RUSTSEC-2026-0194/0195, time crate 스택소진 1건 — RUSTSEC-2026-0009) 3건과 `tauri-utils → urlpattern → unic-*` 경로의 unmaintained 5건, 총 8건 findings가 나오고(`sources ok`, 미승인 소스는 없음) 전부 Tauri 자체의 전이 의존성으로 이 스코프에서 안전한 업그레이드가 없다 — 지금 hard-block하면 이 리포가 만든 적 없는 원인으로 릴리스가 항상 막히므로 release.yml의 게이트는 **report-only**(`continue-on-error: true`)로 배선했다. 각 finding의 실제 도달 가능성(공격자 제어 입력이 그 경로를 타는지)을 심사한 뒤 hard-block으로 승격하는 것이 후속 작업이다 — 검증 없는 block이 harden-runner에서도 피한 것과 같은 실수이기 때문이다. `make supply-chain-check`(로컬 실행)는 그대로 hard-fail이다 — 개발자가 명시적으로 돌려 현재 findings를 보는 용도이므로 완화하지 않는다. |
...
6. ~~oMLX 로컬 추론 런타임의 실기기 검증(D35)~~ **(2026-09-06 실기기 검증 완료)**: M4 Pro/64GB/macOS 26/oMLX 0.6.4/colima(vz)에서 오MLX 실행 파일 발견, `--host 127.0.0.1` 관리형 기동/종료(PID 53467), 모델 로드/파닝(0.5B, 457.8MB), K3s→host TCP 릴레이(127.0.0.1:18000→127.0.0.1:8000, 콜리마 VM에서 `host.lima.internal` 경유 health 200 확인)를 실제 기기 + 설치된 oMLX 바이너리로 관통 검증했다(docs/11-local-inference-runtime.md 런북, docs/12-local-inference-acceptance.md §12, `evidence/local-inference/20260906-ondevice/`). **남은 갭**: 실제 외부망 차단 재실행, warm/ssd-restore 캐시 라벨, 외부 클러스터 L2 브리지, 이 UI를 통한 mlx-lm 폴백 경로는 이 실행에서 검증하지 않았다.
