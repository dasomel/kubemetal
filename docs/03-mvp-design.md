...
6. **oMLX 로컬 추론 런타임의 실기기 검증(D35)**: 오MLX 실행 파일 발견, `--host 127.0.0.1` 관리형 기동/종료, 모델 로드/언로드/파닝/TTL, K3s→host TCP 릴레이(D10 검증 브리지 주소로 바인드)를 실제 Apple Silicon 기기 + 설치된 oMLX 바이너리로 관통 검증하지 않았다. docs/11-local-inference-runtime.md 상단이 명시하듯 정적/CI 검증까지만 완료됐고, 그 문서의 런북(포트/헬스/스트리밍/캐시 evidence)이 실기기에서 통과하기 전까지 "동작한다"고 주장하지 않는다.
