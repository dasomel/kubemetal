...
| --- | --- | --- | --- |
| `describe_deploy_operation` (D39) | `{ action: String }` | `Result<OperationSummary, String>` | 파괴적 액션(`provision_mlops_stack`/`install_kagent`/`start_cluster`/`stop_cluster`) 실행 전, 실제 context/namespace/action/설명을 반환한다. 알 수 없는 action은 Err이며 요약을 추측하지 않는다(D22) |
| `run_gpu_benchmark` | `AppHandle`(암묵) | `Result<GpuBenchmarkResult {gflops, matrix_dim, iterations, python_elapsed_seconds, rust_elapsed_seconds}, String>` | 번들 `gpu_benchmark.py`를 MLX venv로 실행해 실제 matmul 처리량을 반환한다. venv·스크립트·실행·timeout·JSON 파싱 실패는 모두 Err이며 가짜 수치로 폴백하지 않는다 |
| `check_for_orphaned_mlx_processes` | None | `Vec<OrphanedProcessInfo {pid, kind}>` | 이전 앱 종료 후 남은 training/serving PID marker를 확인한다. 죽은 PID marker는 정리하고, 살아 있는 프로세스는 보고만 하며 자동 종료하지 않는다 |
| `get_system_health_summary` | `AppHandle`(암묵) | `Result<SystemHealthSummary, String>` | colima·guardrail·kagent 조회를 병렬 수행한다. 한 하위 조회가 실패해도 다른 결과와 field별 오류를 함께 반환하며, 전체 상태는 기존 필드만으로 healthy/degraded/unknown을 판정한다 |
