...
| `probe_local_inference_live` (Local Inference Runtime) | `{ runtime, endpoint: String, api_key: Option<String> }` | `Result<RuntimeLiveStatus, String>` | 루프백 엔드포인트에 실제 HTTP 헬스/모델 목록 조회 수행 — `loopback_http_request`가 `127.0.0.1`/`localhost` 이외의 호스트는 거부한다 |
...
| `load_omlx_model` (Local Inference Runtime) | `{ endpoint, model_id, api_key: Option<String>, estimated_memory_bytes: Option<u64> }` | `Result<RuntimeActionResult, String>` | 살아있는 런타임의 모델 목록에서 실측 크기를 우선 조회하고 없으면 요청값으로 폴백, `preflight_local_inference_model_load`를 거쳐 Deny면 실제 로드를 호출하지 않고 거부 사유를 리턴, Warn이면 경고를 detail에 덧붙여 로드는 진행한다 |
| `unload_omlx_model` (Local Inference Runtime) | `{ endpoint, model_id, api_key: Option<String>, estimated_memory_bytes: Option<u64> }` | `Result<RuntimeActionResult, String>` | oMLX admin API로 모델 언로드 요청만 중계 — 프리플라이트 없음 |
| `set_omlx_model_settings_sparse` (Local Inference Runtime) | `{ endpoint, model_id, patch: {model_alias, ttl_seconds, is_pinned, is_default}(모두 Option), api_key: Option<String> }` | `Result<RuntimeActionResult, String>` | 파닝/TTL/기본모델/별칭을 sparse 업데이트 — 생략된 필드는 JSON에 아예 넣지 않아(`null`로 직렬화하지 않음) 다른 설정을 초기화하지 않는다. 최소 1개 필드가 있어야 하고 model_id/alias는 형식 검증을 거친다 |
