...
| `get_airgap_status` (Phase 5b) | None | `Result<AirgapStatusReport {airgap_dir, total_assets_count, downloaded_count, total_size_mb, assets: Vec<AirgapAssetItem>}, String>` | `~/.kubemetal/airgap` 하위의 바이너리/차트/이미지 파일 존재 여부와 크기를 실측 조회(`.tar.gz` 우선, 없으면 `.tar` 폴백). D23 |
