...
| `run_gpu_benchmark` (이슈 #11 축소 스코프) | None | `Result<GpuBenchmarkResult {gflops, matrix_dim, iterations, python_elapsed_seconds, rust_elapsed_seconds}, String>` | 앱 venv 파이썬으로 `scripts/mlx/gpu_benchmark.py`(고정 크기 2048×2048 float32 `mx.matmul`, 20회 반복, 매 반복 `mx.eval()` 강제 동기화)를 실행해 실측 GFLOPS를 얻는다. 30초 타임아웃, venv/스크립트 부재·비정상 종료·파싱 실패는 전부 Err — GFLOPS 0이나 가짜 결과로 폴백하지 않는다(D22) |
