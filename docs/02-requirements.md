...
| `check_for_orphaned_mlx_processes` (GitHub #13) | None | `Vec<OrphanedProcessInfo {pid, kind, cmdline}>` | 앱 재시작 시 `~/.kubemetal/mlx-{training,serving}.pid` marker를 읽어 고아 MLX 프로세스를 탐지. pid 생존(kill -0) 확인 후 프로세스 명령줄이 mlx_lm/mlx_vlm인지 검증하여 살아있으면 명령줄을, 확인 불가면 "확인 불가"를 보고(지어내지 않음, D22). 프로세스가 이미 죽었으면 marker만 정리하고 보고하지 않으며, 자동 kill은 하지 않는다 |
