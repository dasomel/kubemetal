...
| `describe_deploy_operation` (이슈 #18 축소 스코프, 2026-08-23) | `{ action: String }` | `Result<OperationSummary, String>` | 파괴적 액션(`provision_mlops_stack`/`install_kagent`/`start_cluster`/`stop_cluster`) 실행 직전 프런트가 호출해 `{ context, namespace, action, target_description }` 확인 요약을 받는다. `start_cluster`/`stop_cluster`는 저장된 배포 대상과 무관하게 항상 colima를 가리킨다(colima 수명주기는 이 앱이 소유하는 유일한 대상, D26). 그 외는 저장된 `DeployTarget`(D26)을 그대로 반영. 알려지지 않은 `action`은 Err — 요약을 지어내지 않는다(D22–D25). 확인 다이얼로그 UI 자체는 이 커맨드의 스코프 밖(별도 결정 필요) |
