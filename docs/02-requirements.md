...
| `describe_deploy_operation` (이슈 #18 축소 스코프, 2026-09-24; `context` 인자 추가로 리뷰 수정) | `{ action: String, context: Option<String> }` | `Result<OperationSummary, String>` | 파괴적 액션(`provision_mlops_stack`/`install_kagent`/`start_cluster`/`stop_cluster`) 실행 직전 프런트가 호출해 `{ context, namespace, action, target_description, risk_class }` 확인 요약을 받는다. 읽기 전용 — kubectl/helm/colima 변경 명령을 실행하지 않는다. `context`는 **InstallKagent에만** 쓰인다 — `install_kagent`(KagentOpsView kubeconfig 선택기)과 정확히 같은 규칙(`resolve_kagent_install_context`)으로 해석해, 명시값이 있으면 그것을, 없으면 저장된 `DeployTarget`(D26)의 컨텍스트로 폴백한다. namespace는 저장된 대상과 무관하게 항상 `KAGENT_NAMESPACE`("kagent") 고정 — kagent는 항상 이 네임스페이스에 설치되기 때문이다. `start_cluster`/`stop_cluster`는 이 인자를 무시하고 저장된 배포 대상과 무관하게 항상 colima를 가리킨다(colima 수명주기는 이 앱이 소유하는 유일한 대상, D26). `provision_mlops_stack`도 이 인자를 무시하고 기존 `get_deploy_target`(D26) 해석 결과를 그대로 반영하며, `provision_mlops_stack` 커맨드가 실행 전 거는 D30 풀스택 게이트(`full_stack_gate`)를 요약도 동일하게 적용한다 — 단 브리지 미검증 거부(`render_args`)는 요약 단계에서 검사하지 않으므로, 요약이 나와도 실행이 그 단계에서 거부될 수 있다 — 대상 해석 실패 시 추측하지 않고 Err(D22). `risk_class`는 대상이 colima면 `local`, 외부 클러스터면 `external`. 알려지지 않은 `action`은 Err — 요약을 지어내지 않는다(D22–D25). 확인 다이얼로그 UI 자체는 이 커맨드의 스코프 밖(별도 결정 필요) |
| `start_port_forward` | None | `Result<String, String>` | MLflow(5001) · SeaweedFS(8333/8888) `kubectl port-forward` 자식 프로세스 기동 (FR-02.2) **(D1 개정 2, 2026-08-06)** 각 잡의 호스트 포트는 고정값이 아니라 기동 시점에 `services::ports`가 배정한다 — 우선 포트(D1)가 비어 있지 않으면 대체 범위의 다음 포트를 잡는다. 배정은 기존 포워드 정리(reap) 이후에 하고, 우선 포트가 풀릴 때까지 최대 2초 기다린다(포트 크리프 방지). 대체 포트가 선택되면 반환 문자열에 `key: 5001 → 5002` 형태로 명시한다. kubectl stderr을 파이프로 잡아 실패 사유를 보존한다 |
| `stop_port_forward` | None | `Result<String, String>` | 추적 중인 포트포워드 자식 프로세스 종료 (FR-02.2) |
| `open_kagent_ui` (2026-08-07) | `{ context: String }` | `Result<String /*url*/, String>` | kagent 패널의 kubeconfig 선택기가 가리키는 컨텍스트의 `svc/kagent-ui`로 포워딩하고 실제 URL을 돌려준다. 배포 대상과 같은 컨텍스트면 대시보드 포워드를 재사용하고, 아니면 컨텍스트별 임시 포워드를 띄운다(호스트 포트는 `services::ports`가 비어 있는 것을 고른다). 2초 후 응답하지 않으면 자식을 정리하고 Err — 적용 성공을 연결 성공으로 보고하지 않는다(D22). 대시보드의 `stop_port_forward`가 이 임시 포워드도 함께 정리한다 |
...
| `start_model_serving` (Phase 2c) | `{ model_path: String, adapter_path: Option<String>, port: u16, runtime: Option<MlxRuntime> }` | `Result<String, String>` | 실기기 실측(2026-07-21): 진입 시 `TcpListener::bind(("127.0.0.1", port))`로 포트 선점 여부를 먼저 확인해 실패하면 spawn 없이 Err(다른 프로세스가 사용 중인 포트 안내, 예: 8080이 Tomcat 등에 선점된 경우). `model_path`가 `adapter_config.json`을 가진 어댑터 디렉터리로 판정되면 그 파일의 `model` 필드에서 베이스 모델 경로를 읽어 자동 승격(베이스를 `--model`, 해당 디렉터리를 `--adapter-path`)하고, 필드가 없으면 Err("베이스 모델을 함께 지정하세요"). 그렇지 않으면 `model_path`를 `--model`로, `adapter_path`가 있으면 `--adapter-path`로 넘겨 venv `python -m mlx_lm server` 기동. 이미 서빙 중이면 Err. 기동한 자식 프로세스는 백그라운드 태스크(`run_serving_reader`)가 `wait()`하며, `stop_model_serving`/`kill_mlx_process`로 의도적으로 멈춘 경우가 아니면 종료 시 `last_serving_error`에 원인을 기록 **(D1 개정 2, 2026-08-06)** 지정 포트가 사용 중이면 실패시키지 않고 8080~8099에서 비어 있는 포트로 자동 대체하며, 요청 포트와 다르면 반환 문자열에 명시한다. 실제 포트는 `ServingStatus.port`에 기록되고 모든 소비자가 그것을 따른다 |
| `stop_model_serving` (Phase 2c) | None | `Result<String, String>` | 진행 중인 모델 서빙 프로세스 종료 |
| `suggest_serving_port` (Phase 2c) | None | `Result<u16, String>` | 8080~8099 범위를 순회하며 `TcpListener::bind(("127.0.0.1", p))`가 성공하는 첫 포트를 리턴(즉시 drop, 제안값일 뿐 예약은 아님). 모두 실패하면 Err. 모델 서빙 카드가 마운트 시 1회 호출해 포트 입력 초기값으로 사용(실패 시 8080 유지, 사용자가 직접 수정하면 그 값을 존중) 탐지는 `0.0.0.0`·`127.0.0.1` 양쪽 바인딩을 요구한다 — 루프백만 보면 와일드카드 점유를 "비어 있음"으로 오판한다 |
| `check_for_orphaned_mlx_processes` (GitHub #13) | None | `Result<OrphanScan {orphans: Vec<OrphanedProcessInfo>, unreadable: Vec<UnreadableMarker>}, String>` | 앱 재시작 시 `~/.kubemetal/mlx-markers/<kind>-<pid>.pid` marker를 읽어 고아 MLX 프로세스를 탐지. pid 생존(kill -0) 확인 후 프로세스 명령줄을 토큰 단위로 검증(`-m mlx_lm*`/`-m mlx_vlm*` 모듈 인자, 또는 basename이 정확히 `finetune_wrapper.py`인 토큰)하여 살아있으면 명령줄을, 확인 불가면 "확인 불가"를 보고(지어내지 않음, D22). 프로세스가 이미 죽었거나 PID가 재사용된 비MLX 프로세스면 marker만 정리하고 보고하지 않으며, 심볼릭 링크나 읽을 수 없는 marker는 unreadable로 보고한다. 현재 세션이 추적하는 학습/서빙 PID(0 제외)는 marker를 보존하고 고아 목록에서 제외하며, 소유권 확인 잠금이 poison 상태이거나 HOME 조회에 실패하면 Err(D22). 자동 kill은 하지 않는다 |
| `get_host_ports` (D1 개정 2) | None | `Result<HashMap<String, u16>, String>` | 현재 배정된 호스트 포트 전체(`mlflow`/`seaweedfs-s3`/`seaweedfs-filer`/`prefect`/`kagent-ui`/`serving`). 배정 전이면 D1 우선값을 돌려준다. 프런트가 링크 URL을 만들 때 쓴다 — 컴포넌트에 포트를 박아두면 대체 포트가 선택됐을 때 링크만 옛 곳을 가리킨다 |
...
| `get_guardrail_status` (Phase 3) | None | `GuardrailStatus {memory_pressure_level, on_battery, battery_pause_enabled, training_paused, caffeinate_active, thermal_state, thermal_pause_enabled, resume_overrides}` | `sysctl -n kern.memorystatus_vm_pressure_level`(D16)과 `pmset -g batt` 실측 기반 하드웨어 가드레일 상태 조회 (FR-05.2/05.3). `resume_overrides`는 수동 재개로 억제된 자동 일시정지 원인 목록(D16 개정 2026-07-29) |
| `set_guardrail_config` (Phase 3) | `{ battery_pause: bool }` | `Result<(), String>` | 배터리 구동 시 학습 자동 일시정지 여부를 `GuardrailState`에 저장 (FR-05.3) |
| `get_system_health_summary` (이슈 #17 축소 스코프) | None | `Result<SystemHealthSummary {colima, colima_error, guardrails, guardrails_error, kagent, kagent_error, overall}, String>` | `get_cluster_status`/`get_guardrail_status`/`get_kagent_diagnostics`를 `tokio::join!`으로 동시 조회해 하나로 모은다. 하위 조회 하나가 실패해도 나머지를 가리지 않는다(D22) — 실패한 필드는 `None` + `*_error` 메시지로 남고 전체 커맨드는 계속 `Ok`를 반환한다. `overall`(`healthy`/`degraded`/`unknown`)은 각 구조체가 이미 가진 필드만으로 판정하며 새 정책을 만들지 않는다: 셋 다 조회 실패면 `unknown`. 조회 실패가 하나라도 있거나 이상 신호가 있으면 `degraded` — colima는 **떠 있을 때만** `kubernetes_active`/`mlflow_ready`/`seaweedfs_ready`도 함께 요구(떠 있지 않으면 그 자체로 이미 불건강), guardrails는 `memory_pressure_level == "critical"` **또는 `"unknown"`**(sysctl 실패, D22 — 측정 못 한 값을 healthy로 흘리지 않는다) 또는 `thermal_state.is_none()`, kagent는 `pod_issues_count > 0`만 본다 — **`kagent_installed == false`는 이상 신호가 아니다**(D30/D33 옵트인 설치, 미설치가 기본 상태). `on_battery`(pmset 실패 시 `false`로 채워지는 미검증 값, guardrails.rs)는 healthy의 근거로 쓰지 않는다. 그 외 `healthy`. dependency graph/correlation ID/redaction/evidence bundle export는 이슈 #17 원안의 스코프 밖 — 별도 설계 결정이 필요하다 |
| `pause_mlx_training` (Phase 3) | None | `Result<bool, String>` | 진행 중인 MLX 학습 pid에 SIGSTOP 전송, `TrainingStatus.status`를 `paused`로 갱신 |
| `resume_mlx_training` (Phase 3) | None | `Result<bool, String>` | 일시정지된 MLX 학습 pid에 SIGCONT 전송, `TrainingStatus.status`를 `running`으로 갱신. `paused_<cause>` 상태에서의 재개는 그 cause를 학습 pid 단위 오버라이드로 기록해 같은 원인의 advisory 신호로 다시 멈추지 않는다 — memory critical은 예외(D16 개정) |
...
| `run_data_ingest` (Phase 5a) | `{ config: IngestConfig }` — `IngestConfig`는 camelCase 필드(`sourceType, sourcePath, collectionName?, embeddingModel?, chunkSize?, chunkOverlap?, enableDvcBackup?, dvcRemoteUrl?, dvcBucket?, overwriteExisting?`) | `Result<IngestFlowResult, String>` | 번들 리소스 `scripts/data/ingest_host.py`를 venv python으로 실행 — extract → clean_chunk → lancedb_index → dvc_backup 4-노드 DAG를 순차 실행해 노드별 상태(`DagNodeState {node_id, name, status, duration_sec, items_processed, details}`) 및 데이터셋 버전/매니페스트 경로(`dataset_version?, manifest_path?`)를 포함한 `IngestFlowResult`를 리턴. `source_type`이 web/rss면 Rust(`validate_ingest_url`)와 Python(`ingest_host.py::_validate_url`) 양쪽에서 scheme allowlist(http/https만)+사설/루프백 호스트 거부(D21) 이중 검증을 수행한다. DVC 백업 크리덴셜은 `dvc_commit_dataset`과 동일하게 env var로 주입 |
...
| `list_ingested_datasets` (Phase 5a) | None | `Result<Vec<IngestedDatasetInfo {collection_name, total_chunks, db_path, is_lance_table, dataset_version?, manifest_path?}>, String>` | LanceDB 디렉터리를 스캔해 `.lance` 서브디렉터리(테이블) 및 `_fallback.json`(lancedb 패키지 미설치 시 폴백) 파일을 데이터셋으로 나열하며, 동반된 매니페스트(`dataset-manifest.json` 또는 `*.manifest.json`)에서 `dataset_version`과 `manifest_path`를 함께 조회 |
...
| `run_gpu_benchmark` (이슈 #11 축소 스코프) | None | `Result<GpuBenchmarkResult {gflops, matrix_dim, iterations, python_elapsed_seconds, rust_elapsed_seconds}, String>` | 앱 venv 파이썬으로 `scripts/mlx/gpu_benchmark.py`(고정 크기 2048×2048 float32 `mx.matmul`, 20회 반복, 매 반복 `mx.eval()` 강제 동기화)를 실행해 실측 GFLOPS를 얻는다. 30초 타임아웃, venv/스크립트 부재·비정상 종료·파싱 실패는 전부 Err — GFLOPS 0이나 가짜 결과로 폴백하지 않는다(D22). 시작 시 비종료 학습(running/paused* 등, 상태·pid 보고) 또는 점유된 서빙 슬롯(pid·port 보고)을 먼저 거부한 뒤 D40 admission(메모리 critical/unknown 또는 thermal pause 활성 상태의 serious/critical)을 적용하며, 상태/설정 잠금 poison도 Err로 거부한다. |
...
| `get_kagent_diagnostics` (Phase 5b) | `{ context: Option<String> }`(기본 `colima`) | `Result<KagentDiagnosticReport {target_context, kagent_ready, kagent_installed, pod_issues_count, recent_diagnosis, recommended_action, active_agents, available_agents}, String>` | `kubectl get pods -n kagent`/`-n default -o json` 2회 실측. `kagent_installed`는 kagent ns에 파드가 1개 이상 존재하는지(Ready 여부와 무관 — 설치 버튼 프롬프트 표시 조건). `kagent_ready`는 `kagent_installed`이면서 **전부 Ready condition=True**일 때만 true(phase만 보면 CrashLoopBackOff도 Running으로 보인다). `active_agents`는 Ready 파드의 `app.kubernetes.io/name` 라벨에서만 수집하고, `recent_diagnosis`는 클러스터가 준 `waiting.reason`/`message`만 인용한다. `available_agents`는 `toggle_kagent_agent`가 실제로 설치 가능한 4종(`services/kagent_agents.rs::TOGGLEABLE_AGENTS`가 단일 출처). 조회 실패는 Err(정상으로 폴백하지 않는다) — D22 |
| `toggle_kagent_agent` (Phase 5b) | `{ agentName: String, enable: bool, context: Option<String> }` | `Result<String, String>` | `enable`이면 내장 Agent CRD 매니페스트(security/promql/observability/rca, `services/kagent_agents.rs`)를 임시 파일로 써 `kubectl apply`, 아니면 `kubectl delete agent.kagent.dev`. 지원 목록 밖의 이름은 Err. 반환 문자열은 kubectl stdout 인용이며 파드 기동 상태를 주장하지 않는다 — 실제 상태는 `get_kagent_diagnostics` 재조회로 확인(D22). rca-agent(#2/#26/#27 승인된 축소 MVP)는 k8s_get_events/k8s_get_resources/k8s_describe_resource로 Warning 이벤트·재시작 파드를 근거로 한 줄짜리 가설만 낸다 — 근거가 없으면 "insufficient evidence"만 출력하고 지어내지 않는다. resources 블록은 두지 않는다(실측 2026-08-26 colima: 256Mi 한도에서 exit 137 OOMKilled — promql/observability-agent와 같은 패턴으로 맞춤) |
| `install_kagent` (Phase 5b, D33) | `AppHandle`(암묵) + `{ context: Option<String> }` | `Result<String, String>` | 지정한 컨텍스트에 kagent를 Helm 설치한다. `context`는 `get_kagent_diagnostics`/`toggle_kagent_agent`와 같은 축(KagentOpsView의 kubeconfig 선택기)이며, 생략하면 저장된 배포 대상(D26)으로 폴백한다 — **D33 개정(2026-08-05)**: 설치만 DeployTarget을 보던 탓에 진단이 narwhal을 보는 화면에서 설치가 colima로 가고 재조회는 계속 "미설치"로 남았다. `kubectl create namespace kagent --dry-run=client -o yaml \| apply`로 먼저 ns를 만들고(`Makefile kagent-up`과 동일 방식 — `helm --create-namespace`는 쓰지 않는다), **`helm upgrade --install kagent-crds oci://ghcr.io/kagent-dev/kagent/helm/kagent-crds --version {kagent-version.txt} -n kagent`를 선행**한 뒤(본 차트 템플릿의 `Agent`/`ModelConfig`/`RemoteMCPServer`는 CRD 없이는 렌더 단계에서 실패한다 — D33 개정 2), `helm upgrade --install kagent oci://ghcr.io/kagent-dev/kagent/helm/kagent --version {kagent-version.txt} -n kagent -f {번들 kagent-values.yaml} --kube-context {ctx} --reuse-values`를 `external_command`(D5)로 실행. 버전은 `scripts/helm/kagent-version.txt`가 단일 출처(D33) — `Makefile`도 같은 파일을 읽는다. Agent CRD와 달리 클러스터에 Mac 의존을 만들지 않아 D30 L1/L2 게이트를 걸지 않는다. 반환은 helm stdout 인용이며 파드 기동을 단정하지 않는다(D22) — 실제 상태는 `get_kagent_diagnostics` 재조회로 확인 **(2026-08-07)** 번들 values는 `providers.openAI.apiKey`(더미)를 반드시 포함한다 — 차트의 `modelconfig-secret.yaml`이 `apiKey`가 있을 때만 `kagent-openai` 시크릿을 만들고, 없으면 ModelConfig가 참조하는 시크릿이 존재하지 않아 **에이전트 파드만** `CreateContainerConfigError`로 죽는다(코어는 Ready라 설치는 성공으로 보인다). 실제 baseUrl/model 연결은 `configure_kagent_model`이 덮어쓴다 |
| `get_kagent_model_status` (Phase 5b, D32) | `AppHandle` + `State<MlxState>`(암묵) | `Result<KagentModelStatus {target_context, target_namespace, gate_ok, gate_reason, serving: Option<{port, model_id}>, model_config: Option<{base_url, model}>, stale_code}, String>` | 저장된 배포 대상(D26, KagentOpsView의 로컬 kubeconfig 선택기와 무관)의 `target.full_stack_gate()`(외부 L1이면 `gate_ok=false`+영문 사유)와 `State<MlxState>.serving`(단일 출처), `kubectl -n kagent get modelconfig default-model-config -o json`(NotFound→`model_config=None`, 그 외 실패→Err)을 종합한다. `stale_code`는 `spec.openAI.baseUrl`이 `http://mac-gpu-service.{namespace}.svc.cluster.local:{port}/v1` 패턴과 얼마나 어긋나는지에 따라 `not_configured`(패턴 자체가 아님·차트 기본값 포함) → `port_mismatch`(포트만 다름) → `model_mismatch`(포트는 맞고 `spec.model`이 `serving.model_id`와 다름) 순으로 판정하고, 전부 일치하면 외부 IP 브리지(D26 `rewrite_bridge_to_endpoints`)의 `kubectl get svc mac-gpu-service -o json`을 best-effort 조회해 `spec.ports[].port`에 서빙 포트가 없으면 `bridge_port_not_proxied`(ExternalName이면 해당 없음 — 포트 미선언이 정상). 전부 일치하면 `stale_code=None`. serving이 없으면 비교 불가로 `stale_code=None`(프런트가 별도 안내) |
| `configure_kagent_model` (Phase 5b, D32) | `AppHandle` + `State<MlxState>`(암묵) | `Result<String, String>` | `target.full_stack_gate()?` → 서빙 없으면 Err("MLX serving is not running — start serving first") → `providers.openAI`(apiKey 더미값·apiKeySecretKey=`OPENAI_API_KEY`·apiKeySecretRef=`kagent-openai`·`config.baseUrl`=D10 브리지 패턴·`model`=`serving.model_path` 전체 경로, basename 금지 — mistakes-log 2026-07-24)를 임시 values 파일(`std::env::temp_dir()`, 처리 후 삭제)로 써 `helm upgrade kagent oci://ghcr.io/kagent-dev/kagent/helm/kagent --version {kagent-version.txt} -n kagent --kube-context {ctx} --reuse-values -f {values}`(`install_kagent`와 소유자 일치 — ModelConfig/Secret 둘 다 helm 소유, `kubectl apply` 금지). `--set` 대신 값 파일을 쓰는 이유는 baseUrl의 `:`/`/`, 경로의 특수문자가 셸 이스케이프 지뢰이기 때문. 반환은 helm stdout 인용 — "연결됨"을 단정하지 않는다(D22), 실제 반영은 `get_kagent_model_status` 재조회로 확인 |
...
| `get_local_inference_status` (Local Inference Runtime, D35) | `State<LocalInferenceState>`(암묵) | `Result<LocalInferenceStatus {preferred_runtime, runtimes: Vec<RuntimeProbe>, managed_process: Option<ManagedRuntimeProcess>, last_exit: Option<RuntimeExitEvidence>}, String>` | oMLX/mlx-lm 두 런타임을 모두 프로브해 설치·능력·버전을 담은 목록과, oMLX가 설치돼 있으면 그것을 우선하고 아니면 mlx-lm으로 떨어지는 `preferred_runtime`을 계산한다. `managed_process`는 KubeMetal이 스폰해 PID를 소유한 프로세스만 노출(다른 프로세스가 이미 같은 포트를 쓰고 있어도 그것을 관리형으로 보고하지 않는다), `last_exit`는 그 프로세스가 예기치 않게 종료됐을 때의 exit code/detail |
| `probe_local_inference_runtime` (Local Inference Runtime) | `{ runtime: LocalInferenceRuntimeKind }` | `Result<RuntimeProbe, String>` | 지정한 런타임(oMLX 또는 mlx-lm) 하나만 발견·버전·능력 프로브 |
| `probe_local_inference_live` (Local Inference Runtime) | `{ runtime, endpoint: String, api_key: Option<String> }` | `Result<RuntimeLiveStatus, String>` | 루프백 엔드포인트에 실제 HTTP 헬스/모델 목록 조회 수행 — `loopback_http_request`가 `127.0.0.1`/`localhost` 이외의 호스트는 거부한다. oMLX 모델 목록은 `/admin/api/models`(세션 쿠키 필요)를 먼저 시도하고, 키 없음/로그인 실패/비-2xx 응답은 모두 `/v1/models`(bearer)로 폴백한다 — 조회는 최선노력이라 실패해도 프로브 자체를 실패시키지 않는다 |
| `list_local_inference_adapters` (Local Inference Runtime) | None | `Result<Vec<RuntimeAdapterDescriptor>, String>` | oMLX/mlx-lm 각 런타임의 표시 이름·기본 포트·능력 선언·설치 안내(brew tap 등)를 정적으로 나열 — 실행 중인 프로세스와 무관 |
| `probe_local_inference_api_capabilities` (Local Inference Runtime) | `{ runtime, endpoint, api_key: Option<String> }` | `Result<ApiCapabilityProbeResult, String>` | OpenAI chat/Anthropic messages/embeddings/rerank 4개 라우트에 최소 페이로드로 실제 요청을 보내 404/405가 아니면 라우트가 존재한다고 판정(`route_present`), 어댑터가 선언한 능력과 대조해 `supported`를 매긴다 |
| `get_local_inference_connection_profile` (Local Inference Runtime) | `{ runtime, endpoint, bridge_endpoint: Option<String>, api_key_configured: bool }` | `Result<ConnectionProfile, String>` | 에이전트/애플리케이션이 붙일 OpenAI/Anthropic base URL과 환경변수 라인을 구성. 엔드포인트는 loopback(`127.0.0.1`/`localhost`)만 허용하고, 실제 API 키 값은 절대 담지 않고 `<set-at-runtime>` 플레이스홀더만 넣는다 |
| `get_local_inference_diagnostics` (Local Inference Runtime) | None | `Result<LocalInferenceDiagnostics, String>` | `~/.kubemetal/logs/omlx.log` 꼬리(최대 512KiB)에서 Metal OOM/prefill 가드/캐시 재사용/eviction 시그니처를 역순으로 찾아 코드화된 finding으로 리턴, `iogpu.wired_limit_mb`/`hw.memsize`도 함께 조회 |
| `preflight_local_inference_model_load` (Local Inference Runtime) | `AppHandle`(암묵) + `{ model_id: String, estimated_memory_bytes: Option<u64> }` | `Result<ModelLoadPreflight, String>` | 메모리 압력(`critical`→Deny, `warn`/조회불가→Warn)·발열(`critical`/`serious`→Deny, `fair`→Warn)·MLX 파인튜닝 진행 여부·추정 메모리가 Metal wired limit의 90%를 넘는지를 종합해 `Allow`/`Warn`/`Deny`를 판정. `load_omlx_model`이 이 판정을 먼저 거친 뒤 실제 로드를 수행한다 |
| `evaluate_local_inference_cache_compatibility` (Local Inference Runtime) | `{ previous: Option<CacheIdentity>, current: CacheIdentity }` | `Result<CacheCompatibilityResult, String>` | runtime/model id/revision/digest/quantization/cache format version 7개 필드를 비교해 캐시 재사용 가능 여부를 판정 — 하나라도 다르거나 이전 값이 없으면 `compatible=false`/`None`이고 "cold로 취급, 재사용을 주장하지 않는다"는 액션을 리턴한다(캐시 hit 자체를 이 커맨드가 확인하지는 않는다) |
| `start_local_inference_runtime` (Local Inference Runtime, D35) | `AppHandle`(암묵) + `RuntimeLaunchConfig {runtime, port, model_dir, cache_enabled, paged_ssd_cache_dir, paged_ssd_cache_max_size, hot_cache_max_size, max_concurrent_requests, memory_guard_tier}` + `State<LocalInferenceState>`(암묵) | `Result<ManagedRuntimeProcess, String>` | 현재 oMLX만 지원(mlx-lm은 기존 MLX 스튜디오 서빙 경로가 소유). 이미 관리 중인 프로세스가 있으면 Err, 요청 포트에 이미 응답하는 서버가 있으면 소유권을 가져가지 않고 Err. `model_dir`/`paged_ssd_cache_dir`는 홈 디렉터리 하위로 제한하고, canonicalize 검증은 `spawn_blocking`으로 async 런타임 밖에서 수행한다(2026-09-05). 항상 `--host 127.0.0.1`로 기동하고 로그는 `~/.kubemetal/logs/omlx.log`에 append. 자식 프로세스는 백그라운드 태스크가 `wait()`하며 예기치 않게 종료하면 `LocalInferenceState.last_exit`에 근거를 남긴다. `memory_guard_tier`는 `Option<String>`으로 `{off,safe,balanced,aggressive}` 중 하나만 허용한다 — `omlx serve --memory-guard`는 값 없이 호출하면 argparse에서 실패한다(실측 2026-09-06). `pinned_models`는 존재하지 않는다: `omlx serve`에 `--pin` 플래그가 없고, 파닝은 기동 후 `set_omlx_model_settings_sparse`가 admin API로 수행한다 |
| `stop_local_inference_runtime` (Local Inference Runtime) | `State<LocalInferenceState>`(암묵) | `Result<RuntimeActionResult, String>` | KubeMetal이 소유한 관리형 프로세스에만 SIGTERM을 보내고 5초(100ms×50) 폴링으로 종료를 확인, 못 끝내면 소유권을 유지한 채 Err |
| `load_omlx_model` (Local Inference Runtime) | `{ endpoint, model_id, api_key: Option<String>, estimated_memory_bytes: Option<u64> }` | `Result<RuntimeActionResult, String>` | 살아있는 런타임의 모델 목록에서 실측 크기를 우선 조회하고 없으면 요청값으로 폴백, `preflight_local_inference_model_load`를 거쳐 Deny면 실제 로드를 호출하지 않고 거부 사유를 리턴, Warn이면 경고를 detail에 덧붙여 로드는 진행한다. `/admin/api/models/{id}/load`는 세션 쿠키 인증이라(bearer 아님, 실측 2026-09-06) `omlx_admin_session`으로 로그인해 쿠키를 얻은 뒤 호출 — 키가 비어 있으면 오MLX를 호출하지 않고 즉시 안내 오류를 리턴한다 |
| `unload_omlx_model` (Local Inference Runtime) | `{ endpoint, model_id, api_key: Option<String>, estimated_memory_bytes: Option<u64> }` | `Result<RuntimeActionResult, String>` | oMLX admin API로 모델 언로드 요청만 중계 — 프리플라이트 없음. `load_omlx_model`과 동일하게 세션 쿠키로 인증한다 |
| `set_omlx_model_settings_sparse` (Local Inference Runtime) | `{ endpoint, model_id, patch: {model_alias, ttl_seconds, is_pinned, is_default}(모두 Option), api_key: Option<String> }` | `Result<RuntimeActionResult, String>` | 파닝/TTL/기본모델/별칭을 sparse 업데이트 — 생략된 필드는 JSON에 아예 넣지 않아(`null`로 직렬화하지 않음) 다른 설정을 초기화하지 않는다. 최소 1개 필드가 있어야 하고 model_id/alias는 형식 검증을 거친다. `PUT /admin/api/models/{id}/settings` 역시 세션 쿠키 인증이라 호출 전에 `omlx_admin_session`으로 로그인한다 |
| `get_local_inference_bridge_status` (Local Inference Runtime, D35) | `State<LocalInferenceBridgeState>`(암묵) | `Result<LocalInferenceBridgeStatus {running, bind_address, target_address}, String>` | 현재 KubeMetal이 띄운 K3s→host TCP 릴레이의 실행 여부와 바인드/타깃 주소 조회 |
| `start_local_inference_bridge` (Local Inference Runtime, D35) | `{ bind_host: String, bind_port: u16, target_port: u16 }` + `State<LocalInferenceBridgeState>`(암묵) | `Result<LocalInferenceBridgeStatus, String>` | `bind_host`는 loopback이거나 배포 대상(D26/D10)이 실측 검증한 브리지 주소(`BridgeState::Verified{host}`)와 정확히 일치해야 하며(2026-09-05 강화 — 이전에는 임의의 사설/link-local 주소를 허용해 인증 없는 릴레이가 사용자의 실제 LAN 인터페이스에 뜰 수 있었다), 그 외는 거부한다. 타깃은 요청 포트와 무관하게 항상 `127.0.0.1:<target_port>`로 고정. 바이트 그대로 중계하는 바이다이렉셔널 TCP 릴레이라 SSE/스트리밍도 그대로 통과한다. 이미 실행 중이면 Err |
| `stop_local_inference_bridge` (Local Inference Runtime) | `State<LocalInferenceBridgeState>`(암묵) | `Result<LocalInferenceBridgeStatus, String>` | 실행 중인 릴레이 태스크를 abort. 실행 중이 아니면 Err |
| `inspect_local_inference_cache` (Local Inference Runtime) | `{ path: String }` | `Result<CacheInspection {path, exists, bytes, files, directories, partial, errors}, String>` | 경로는 홈 디렉터리 하위로 canonicalize 검증(2026-09-05: 검증과 재귀 스캔 모두 `spawn_blocking`으로 async 런타임 밖에서 수행) 후 심볼릭 링크를 따라가지 않고 재귀 스캔해 파일/디렉터리 수와 총 바이트를 집계. 개별 항목 조회 실패는 `partial=true`+`errors`에 누적하고 전체를 중단하지 않는다 |
| `cleanup_local_inference_cache` (Local Inference Runtime) | `{ path: String, max_age_hours: u64, dry_run: bool(기본 true) }` | `Result<CacheCleanupResult, String>` | `max_age_hours`는 1 이상이어야 하고, 경로는 홈 하위이면서 경로 성분에 `cache`/`kv-cache`/`kv_cache`가 포함된 디렉터리로만 제한(HOME 자체 정리 거부). `dry_run=true`(기본)면 삭제 대상만 집계, `false`면 mtime이 cutoff보다 오래된 파일과 빈 디렉터리를 실제로 제거. 검증과 정리 모두 `spawn_blocking`으로 수행(2026-09-05) |
