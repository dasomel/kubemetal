...
# kustomize drops comments, so the seaweedfs-s3-credentials Secret arrives in the
# rendered file as a bare access-key/secret-key pair with nothing saying what it is.
# That file is committed to the public narwhal repo, where a reader — or a secret
# scanner — cannot tell a placeholder from a live credential. The values themselves
# are deliberately NOT changed: D21 (2026-07-24) records that they were made to match
# the Rust/Python fallbacks in access.rs, ingest_host.py and rag_host.py, and a
# rename that misses one reintroduces exactly the desync that lesson was about.
# So state the context in the output instead.
cat <<'HEADER'
# Generated by scripts/k8s/render.sh in the kubemetal repo — do not edit here.
#
# NOTE on the seaweedfs-s3-credentials Secret below: those are placeholders, not
# secrets. The SeaweedFS S3 gateway runs with IAM unconfigured and accepts any
# credential; the values exist only to satisfy the signature headers boto3 and the
# MLflow S3 client require. They are duplicated as fallback defaults in the kubemetal
# repo (access.rs, ingest_host.py, rag_host.py) and must stay identical to those.
HEADER
